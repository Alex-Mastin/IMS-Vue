<template>
    <div>
        <div class="item-title">
            <label class="item-title-text font-sb">Products</label>
        ...
        <div class="details">
            <table id="product-table">
                ...
                <tr class="noRecords" v-if="lines.length === 0">
                    <td class="text-muted text-center height-100" colspan="4">No Records Found</td>
                </tr>
                </thead>
                <tbody id="table">
                <tr v-for="(line, index) in lines" :key="index">
                    <td>
                        <input type="text" v-if="line.edit === true" v-model="lines[index].product" @input="line.changed = true">
                        ...
                        <button v-if="line.edit === true" class="action-btn right" @click="valid(line.product, line.changed, line.original) ? (line.edit = false, line.changed = false, line.original = line.product) : line.edit = true">
                            <!--(valid(product, model, capacity, msrp) ? (line.edit = false) : line.edit = true) :-->
                            <!--(valid(line.product, line.model, line.capacity, line.msrp) ? line.edit = false : line.edit = true)">-->
    ...
    export default {
        ...
        methods: {
            ...
            deleteLine(e) {
                ...
                let product = row.children[0].firstChild.value;
                let docID = this.normalize(product);

                // Model and capacity are defined
                if (product) {
                    if (confirm("Do you want to delete " + product + "?")) {
                        database.collection('product').doc(docID).delete()
                            .then(() => {
            ...
            valid(product, changed, original) {
                ...
                if (changed && product){
                    ...
                    if (original) {
                        ...
                        database.collection('product').doc(originalDoc).delete()
                            .then(function () {
                    ...
                    database.collection('product').doc(docID).set({
                        product: product,
                        edit: false,
                        changed: false,
                        original: product
