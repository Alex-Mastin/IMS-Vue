<template>
    <div>
        <div class="item-title">
            <label class="item-title-text font-sb">Manufacturers</label>
            ...
            <table id="manufacturer-table">
                ...
                <tr v-for="(line, index) in lines" :key="index">
                    <td>
                        ...
                        <button v-if="line.edit === true" class="action-btn right" @click="valid(line.manufacturer, line.changed, line.original) ? (line.edit = false, line.changed = false, line.original = line.manufacturer) : line.edit = true">
    ...
    export default {
        ...
        methods: {
            ...
            deleteLine(e, id) {
                ...
                let manufacturer = row.children[0].firstChild.value;
                let docID = this.normalize(manufacturer);

                // Model and capacity are defined
                if (manufacturer) {
                    if (confirm("Do you want to delete " + manufacturer + "?")) {
                        database.collection('manufacturer').doc(docID).delete()
                            .then(() => {
                                this.lines = this.lines.filter(line => {
                                    return line.manufacturer !== manufacturer;
                                })
                            })
                    }
                }
            ...
            valid(manufacturer, changed, original) {
                ...
                if (changed && manufacturer){
                    ...
                    if (original) {
                        ...
                        database.collection('manufacturer').doc(originalDoc).delete()
                            .then(function () {
                    ...
                    database.collection('manufacturer').doc(docID).set({
